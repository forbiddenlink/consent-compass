/**
 * Cookie Auto-Categorization
 *
 * Provides automatic categorization of cookies using multiple strategies:
 * 1. Open Cookie Database exact matches
 * 2. Open Cookie Database prefix patterns
 * 3. Local regex patterns for common cookies
 * 4. Domain-based heuristics
 *
 * Categories follow GDPR/ePrivacy standards:
...
import type { CookieCategory, CategorizedCookie } from "@/lib/types";
import { lookupCookieExact, lookupCookieByPrefix, type CookieEntry } from "./database";

/**
 * Cookie pattern definition for local categorization rules
 */
...
/**
 * Local fallback patterns for cookies not in Open Cookie Database
 * These are checked after the OCD lookup fails
 */
const LOCAL_PATTERNS: CookiePattern[] = [
  // ===== NECESSARY =====
  // Session & Security
...
/**
 * Domain patterns for tracker identification
 */
const TRACKER_DOMAINS: Array<{ pattern: RegExp; category: CookieCategory; vendor: string }> = [
  { pattern: /doubleclick/i, category: "marketing", vendor: "Google Ads" },
  { pattern: /googlesyndication/i, category: "marketing", vendor: "Google Ads" },
  { pattern: /facebook\.com/i, category: "marketing", vendor: "Facebook" },
  { pattern: /fbcdn/i, category: "marketing", vendor: "Facebook" },
  { pattern: /google-analytics/i, category: "analytics", vendor: "Google Analytics" },
  { pattern: /googletagmanager/i, category: "analytics", vendor: "Google Tag Manager" },
  { pattern: /linkedin\.com/i, category: "marketing", vendor: "LinkedIn" },
  { pattern: /twitter\.com/i, category: "marketing", vendor: "Twitter" },
  { pattern: /x\.com/i, category: "marketing", vendor: "Twitter" },
  { pattern: /tiktok\.com/i, category: "marketing", vendor: "TikTok" },
  { pattern: /pinterest\.com/i, category: "marketing", vendor: "Pinterest" },
  { pattern: /snapchat\.com/i, category: "marketing", vendor: "Snapchat" },
  { pattern: /criteo/i, category: "marketing", vendor: "Criteo" },
  { pattern: /adroll/i, category: "marketing", vendor: "AdRoll" },
  { pattern: /hotjar/i, category: "analytics", vendor: "Hotjar" },
  { pattern: /mixpanel/i, category: "analytics", vendor: "Mixpanel" },
  { pattern: /segment\.com/i, category: "analytics", vendor: "Segment" },
  { pattern: /amplitude/i, category: "analytics", vendor: "Amplitude" },
  { pattern: /fullstory/i, category: "analytics", vendor: "FullStory" },
  { pattern: /logrocket/i, category: "analytics", vendor: "LogRocket" },
];

/**
 * Result of categorizing a single cookie
 */
export interface CategorizationResult {
  category: CookieCategory;
  vendor?: string;
  description?: string;
  source: "ocd-exact" | "ocd-prefix" | "local-pattern" | "domain-heuristic" | "unknown";
  confidence: number; // 0-1
}

/**
 * Categorize a single cookie using multiple strategies
 *
 * Lookup order:
 * 1. Open Cookie Database exact matches (highest confidence)
 * 2. Open Cookie Database prefix patterns
 * 3. Local regex patterns
 * 4. Domain-based heuristics
 * 5. Return unknown (lowest confidence)
 */
export function categorizeCookieFull(name: string, domain?: string): CategorizationResult {
  // 1. Check Open Cookie Database exact matches
  const exactMatch = lookupCookieExact(name);
  if (exactMatch) {
    return {
  ...
  // 2. Check Open Cookie Database prefix patterns
  const prefixMatch = lookupCookieByPrefix(name);
  if (prefixMatch) {
    return {
      category: prefixMatch.category,
      vendor: prefixMatch.vendor,
  ...
  // 3. Check local patterns
  for (const entry of LOCAL_PATTERNS) {
  ...
  if (domain) {
    for (const tracker of TRACKER_DOMAINS) {
      if (tracker.pattern.test(domain)) {
        return {
...
/**
 * Simplified categorization (backward compatible)
 */
...
/**
 * Get categorization confidence for a set of cookies
 */
export function getCategorizationConfidence(cookies: Array<{ name: string; domain?: string }>): {
  ...
  return {
    averageConfidence: cookies.length > 0 ? totalConfidence / cookies.length : 0,
    highConfidenceCount,
    lowConfidenceCount,
    unknownCount,
  };
